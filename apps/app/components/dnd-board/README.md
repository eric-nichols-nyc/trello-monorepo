...
- **`demo-sortable-column.tsx` / `demo-sortable-card.tsx`** — Minimal sortable UI (not `BoardColumn` / `BoardCardItem`).
...
3. Swap **demo chrome** for real components (`ListHeader`, `ListFooter`, `BoardCardItem`, etc.) once behavior matches.
